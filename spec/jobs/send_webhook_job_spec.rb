...
RSpec.describe SendWebhookJob, type: :job do
  ...
  context 'when webhook_type is credit_note.provider_refund_failure' do
    let(:webhook_service) { instance_double(Webhooks::PaymentProviders::CreditNoteRefundFailureService) }
    let(:credit_note) { create(:credit_note) }
    ...
    before do
      allow(Webhooks::PaymentProviders::CreditNoteRefundFailureService).to receive(:new)
    ...
    it 'calls the webhook service' do
      ...
      expect(Webhooks::PaymentProviders::CreditNoteRefundFailureService).to have_received(:new)
      expect(webhook_service).to have_received(:call)
    end
  end
